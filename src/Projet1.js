...
import moinschereImg from "./assets/moinschere.webp";
...
function Projet1() {
  ...
  // Animation simple au scroll : translation verticale douce
  useEffect(() => {
    const handleScroll = () => {
      const sections = document.querySelectorAll('section');
      sections.forEach(sec => {
        const rect = sec.getBoundingClientRect();
        const offset = Math.max(-30, Math.min(30, (window.innerHeight / 2 - rect.top) * 0.08));
        sec.style.transform = `translateY(${offset}px)`;
        sec.style.transition = 'transform 0.4s cubic-bezier(.22,1,.36,1)';
      });
    };
      ...
      .popup-overlay {
        ...
        * {
        ...
        @media (max-width: 768px) {
          ...
          /* Sur mobile, rendre le cadre un peu plus grand que la version précédente */
          .video-container {
            max-width: 100%;
            aspect-ratio: 16 / 9;
            max-height: 360px; /* mobile: un peu plus grand */
            border-radius: 10px;
          }
